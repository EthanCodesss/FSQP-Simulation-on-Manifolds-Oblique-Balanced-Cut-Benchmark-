...
%           January, 20, 2020: Write the code.
    ...
    localdefaults.mus = ones(condet.n_ineq_constraint_cost, 1); % the Lagrangian multiplier
    localdefaults.lambdas = ones(condet.n_eq_constraint_cost, 1); % the Lagrangian multiplier   
    % Linesearch
    ...
    function gradLag = gradLagrangian(x, mus, lambdas)
        ...
        if condet.has_ineq_cost
            % n_ineq_constraint_cost为不等式约束的cost的个�?
            for numineq = 1: condet.n_ineq_constraint_cost
    ...
    function val = KKT_residual(xCur, mus, lambdas)
        ...
        % 固定秩流形检查，如果优化问题的流形涉及矩阵秩，检查当前点的矩阵秩是否与预期�?�一�?
        % 如果不一致，将残差�?�设为无穷大
        if contains(problem0.M.name(),'rank')
